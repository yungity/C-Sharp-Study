...
+ private 접근 한정자인 필드 myField는 외부에서 접근하려면(main 등) GetMyfield(), SetField() 메소드를 만들어 접근해야함.★

```

- 이 때 __프로퍼티__ 를 이용

:★ __은닉성을 지키고 필드를 추가할 때마다 필드에 값을 할당하는 메소드/저장된 값을 불러오는 메소드를 선언하지 않고 편리하게 사용 가능.__★

### 프로퍼티
: 사물/ 개체의 특징/속성/성질/변수 등 추상화 시 특징짓는 것들로 나타내며, 객체들이 고유하게 유지하는 데이터이다.
- '이름(name) : 값(value)' 형식으로 표현을 가짐.

### 프로퍼티와 어트리뷰트의 차이
- 프로퍼티 : '이름 : 값' 쌍으로 표현되는 동적인 속성으로 조작 가능함을 주로 의미.
...
- 원래 무명 형식을 쓰지 않고 클래스 선언 후 프로퍼티 이용 필드 초기화
...
: 메소드와 함께 프로퍼티나 인덱서도 가질 수 있다.

인터페이스를 상속하는 클래스는 반드시 이들을 구현해줌. (인터페이스를 상속하여 구현할 때는 override 키워드는 사용하지 않음. 자동구현도 써도됨)

### 주의 
: __인터페이스에서의 프로퍼티 선언__ 도 get, set의 구현을 갖지 않아서 __클래스의 자동 구현 프로퍼티 선언__ 모습과 동일함. ( { get; set; } 만) 

-> __컴파일러는 인터페이스 선언에서는 구현이 없는 걸 알고 있어서 구현하지 않음.__
...
: 클래스처럼 __자동 구현 프로퍼티__ 와 인터페이스처럼 __구현되지 않은 프로퍼티(추상프로퍼티)__ 를 가질 수 있다.

### 추상 프로퍼티
: 추상 클래스의 프로퍼티. 구현되지 않은 상태로 선언된다. 추상 클래스를 상속 받은 파생클래스가 해당 프로퍼티를 구현( __override__ )하도록 강제하기 위해.

- 추상 클래스는 모습이 같은 __자동 구현 프로퍼티__ 와 __추상 프로퍼티__ (get; set; )를 구별해 주기 위해 구현이 아닌 인터페이스의 추상프로퍼티 선언임을 __abstract 한정자로 구분.__ 

### 추상 프로퍼티의 선언
- __abstract 한정자__

: 추상 프로퍼티 맨 앞에 __abstract__ 한정자를 붙여줌. 

- __override 한정자__

-> 상속받은 클래스에서 구현되지 않는 추상 프로퍼티를 구현할 때는 접근 한정자 뒤에 __override__ 를 붙여줌.

(** 그냥 상속받은 인터페이스의 메서드를 구현할 때는 override 키워드는 사용하지 않음)
